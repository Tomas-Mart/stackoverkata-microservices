...
# StackOverKata Microservices System

🌟 Обзор системы

### Микросервисная платформа для управления аутентификацией, профилями и ресурсами.

## Архитектура:

### Сервис Eureka для регистрации микросервисов

### 3 независимых сервиса (Auth, Profile, Resource)

### Интеграция с PostgreSQL (prod) и H2 (local)

### Управление конфигурацией через Spring Profiles
...
Автомиграции: Hibernate автоматически создаёт схемы
Liquibase: Отключён
Зависимости: Не требуется Docker или PostgreSQL
...
Миграции: Управление через Liquibase
...
DB_HOST=postgres-resource
DB_PORT=5432
DB_NAME=stackover_resource_service
DB_USER=postgres
DB_PASSWORD=root

# Eureka
EUREKA_CLIENT_SERVICEURL_DEFAULTZONE=http://stackover-eureka-server:8761/eureka/
🚀 Запуск системы

### Вариант 1: Docker Compose (dev профиль)

Шаг 1: Сборка образов

bash
# Собрать все сервисы
docker-compose -f docker-compose.yml -f docker-compose-resource.yml build

# Собрать конкретный сервис (пример)
docker-compose -f docker-compose.yml build auth-service

Шаг 2: Запуск системы

bash
# Полный запуск с фоновым режимом
docker-compose -f docker-compose.yml -f docker-compose-resource.yml up -d

# Запуск с выводом логов
docker-compose -f docker-compose.yml -f docker-compose-resource.yml up
Шаг 3: Проверка статуса

bash
docker-compose ps

### Вариант 2: Локальный запуск (local профиль)

Для Eureka Server:

В IDE откройте stackoverkata-eureka-server
Добавьте конфигурацию запуска:
Main Class: org.springframework.boot.loader.JarLauncher
VM Options: -Dspring.profiles.active=local

Для микросервисов (Auth/Profile/Resource):

Запустите Eureka Server

Для каждого сервиса:
Откройте модуль в IDE
Добавьте VM Options: -Dspring.profiles.active=local
Запустите через main() метод
...
Укажите для каждого сервиса:

Параметр	Значение
Main class	com.example.Application
VM options	-Dspring.profiles.active=local
Use classpath	Модуль сервиса
...
Profile Service	http://localhost:8181	-
Resource Service	http://localhost:8182	-
H2 Console (local)	http://localhost:8180/h2-console	JDBC URL: jdbc:h2:mem:resource_db
...
bash
# Запуск с кастомными параметрами БД
docker-compose run -e DB_PASSWORD=mysecret -e DB_HOST=my-postgres auth-service

# Запуск с другим профилем
ACTIVE_PROFILE=local docker-compose up

Расширенный пример .env:
...
Инстансы отображаются с корректными IP
Eureka Dashboard
...
docker exec -it postgres-auth psql -U postgres -d stackover_auth_service
...
Для сброса: перезапустите сервис
...
Миграции выполняются при старте в dev профиле
Для отключения: spring.liquibase.enabled=false
Проверка истории: SELECT * FROM databasechangelog;
...
bash
# Проверьте connectivity между контейнерами
docker exec -it auth-service curl http://stackover-eureka-server:8761

# Убедитесь в правильности переменных:
...
docker exec postgres-auth pg_isready -U postgres

# Проверьте логи PostgreSQL
docker logs postgres-auth | grep -i 'connection'

✅ Преимущества системы

Особенность	- Преимущество
Профильная конфигурация	- Разделение dev/test/prod окружений
Docker-оркестрация	- Быстрое развёртывание
Автоматические миграции	- Контроль версий схемы БД
Сервис-регистрация	- Динамическое обнаружение сервисов
...
# AdviceRestController - Описание работы

## @RestControllerAdvice - что это?

@RestControllerAdvice - это специализированная аннотация Spring, которая объединяет @ControllerAdvice и @ResponseBody, позволяя обрабатывать исключения во всем приложении в одном месте и возвращать структурированные ответы в формате JSON.

## Как работает AdviceRestController?

Глобальная обработка исключений: Класс перехватывает исключения, выбрасываемые в любом месте приложения, и преобразует их в стандартизированные HTTP-ответы.
...
error - текстовое описание статуса

message - сообщение из исключения

path - URL, по которому произошла ошибка

## Обрабатываемые исключения:

TagNotFoundException - 404 Not Found

AccountExistException - 409 Conflict

AccountNotAvailableException - 401 Unauthorized

FeignRequestException - 500 Internal Server Error

TagAlreadyExistsException - 409 Conflict

QuestionException, AnswerException, CommentAnswerException, ConstrainException, VoteException, ApiRequestException - 400 Bad Request
...
Чистый код контроллеров (без try-catch блоков)

Стандартизированный формат ответов об ошибках

Легкая расширяемость для новых типов исключений

## Пример ответа

При вызове несуществующего тега (TagNotFoundException):
...
Такой подход значительно улучшает опыт взаимодействия с API, предоставляя клиентам понятные и структурированные сообщения об ошибках.
